...
function FilterDrawerItem(props) {
    ...
    const filterFunction = props.filterItem.filterFunction;

    return (
        <Grid container item direction="column" sx={{ mt: '10px' }}>
            <Grid container item spacing={1} direction="row">
                <Grid item>
                    <Typography color='black.dark' variant='body1'>{filterItemName}: </Typography>
                </Grid>
                <Grid item>
            ...
            <FormGroup sx={{ml:'10px'}}>
                {filterDrawerOptions.map((option)=>{
                    return(<FormControlLabel componentsProps={{typography:{color:'black.dark', variant:"caption"}}} control={<Checkbox size="small" color="blue"/>} label={option} />)
                })}
            </FormGroup>
        </Grid>
    )
